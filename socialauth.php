...
if ($token == '' || $token == null || $token !== WISession::get('WI_social_token')) {
...
require_once 'WICore/WIVendor/hybridauth/Hybrid/Auth.php';

$config = 'WICore/WIVendor/hybridauth/config.php';
...
try {
    ...
    if ($validator->emailExist($userProfile->email)) {
        ...
        $db->insert('WI_Members', $info);

        $userId = $db->lastInsertId();

        $details['user_id'] = $userId;

        $db->insert('WI_user_details', $details);
